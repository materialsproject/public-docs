...
# Thermodynamic Stability & Equilibria

## Total Energy Adjustments

To better model energies across diverse chemical spaces, we apply several adjustments to the total energy. These adjustments fall into two categories, each of which is briefly described below. Our correction scheme assumes independent, linear corrections associated with each corrected element. For example, $$\text{VO}_2$$ would receive both a '$$\text{V}$$' and an 'oxide' correction (as explained below), while elemental $$\text{V}$$ would receive no corrections. For complete details of our correction scheme, refer to Wang et al. [\[1\]](./#references)
...
### GGA / GGA+_U_ Mixing Corrections
...
Specifically, we use GGA+_U_ for oxide and fluoride compounds containing any of the transition metals $$\text{V}$$, $$\text{Cr}$$, $$\text{Mn}$$, $$\text{Fe}$$, $$\text{Co}$$, $$\text{Ni}$$, $$\text{W}$$, and $$\text{Mo}$$, and GGA for everything else. More details on this method can be found in refs. [\[1,2\]](./#references)
